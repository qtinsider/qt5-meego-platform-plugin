...
#include "mplatformbackingstore.h" // for createSystemVShmSegment()
...
XcbBasicConnection::XcbBasicConnection(const char *displayName)
    : m_displayName(displayName ? QByteArray(displayName) : qgetenv("DISPLAY"))
{
    ...
    xcb_extension_t *extensions[] = {
        &xcb_shm_id, &xcb_xfixes_id, &xcb_shape_id, &xcb_sync_id,
        &xcb_render_id, &xcb_input_id, nullptr
    };
...
XcbBasicConnection::~XcbBasicConnection()
{
    if (isConnected()) {
...
size_t XcbBasicConnection::maxRequestDataBytes(size_t requestSize) const
{
    if (hasBigRequest())
...
xcb_atom_t XcbBasicConnection::internAtom(const char *name)
{
    if (!name || *name == 0)
        return XCB_NONE;
...
QByteArray XcbBasicConnection::atomName(xcb_atom_t atom)
{
    if (!atom)
        return QByteArray();
...
bool XcbBasicConnection::hasBigRequest() const
{
...
bool XcbBasicConnection::isXIEvent(xcb_generic_event_t *event) const
{
    qt_xcb_ge_event_t *e = reinterpret_cast<qt_xcb_ge_event_t *>(event);
    return e->extension == m_xiOpCode;
}

bool XcbBasicConnection::isXIType(xcb_generic_event_t *event, uint16_t type) const
{
    if (!isXIEvent(event))
        return false;
...
bool XcbBasicConnection::isXFixesType(uint responseType, int eventType) const
{
    return m_hasXFixes && responseType == m_xfixesFirstEvent + eventType;
}

void XcbBasicConnection::initializeXSync()
{
...
void XcbBasicConnection::initializeShm()
{
    ...
    if (!MPlatformBackingStore::createSystemVShmSegment(m_xcbConnection)) {
...
void XcbBasicConnection::initializeXRender()
{
...
void XcbBasicConnection::initializeXFixes()
{
...
void XcbBasicConnection::initializeXInput2()
{
...
void XcbBasicConnection::initializeXShape()
{
