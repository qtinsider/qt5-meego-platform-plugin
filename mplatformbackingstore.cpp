...
class MPlatformBackingStore;

class XcbBackingStoreImage : public XcbObject
{
public:
    XcbBackingStoreImage(MPlatformBackingStore *backingStore, const QSize &size);
    XcbBackingStoreImage(MPlatformBackingStore *backingStore, const QSize &size, uint depth, QImage::Format format);
    ~XcbBackingStoreImage() { destroy(true); }
...
private:
    ...
    MPlatformBackingStore *m_backingStore = nullptr;
...
class MPlatformGraphicsBuffer : public QPlatformGraphicsBuffer
{
public:
    MPlatformGraphicsBuffer(QImage *image)
...
XcbBackingStoreImage::XcbBackingStoreImage(MPlatformBackingStore *backingStore, const QSize &size)
    : XcbObject(backingStore->connection())
    , m_backingStore(backingStore)
{
    auto window = static_cast<MPlatformWindow *>(m_backingStore->window()->handle());
...
void XcbBackingStoreImage::init(const QSize &size, uint depth, QImage::Format format)
{
...
void XcbBackingStoreImage::resize(const QSize &size)
{
    destroy(false);
    ...
    m_graphics_buffer = new MPlatformGraphicsBuffer(&m_qimage);

    m_xcb_pixmap = xcb_generate_id(xcb_connection());
    auto xcbScreen = static_cast<MPlatformScreen *>(m_backingStore->window()->screen()->handle());
...
void XcbBackingStoreImage::destroy(bool destroyShm)
{
    if (m_xcb_image) {
        if (m_xcb_image->data) {
            if (m_shm_info.shmaddr) {
...
void XcbBackingStoreImage::flushScrolledRegion(bool clientSideScroll)
{
...
void XcbBackingStoreImage::createShmSegment(size_t segmentSize)
{
...
bool XcbBackingStoreImage::createSystemVShmSegment(xcb_connection_t *c, size_t segmentSize,
...
void XcbBackingStoreImage::destroyShmSegment()
{
...
bool XcbBackingStoreImage::scroll(const QRegion &area, int dx, int dy)
{
...
void XcbBackingStoreImage::ensureGC(xcb_drawable_t dst)
{
...
void XcbBackingStoreImage::shmPutImage(xcb_drawable_t drawable, const QRegion &region, const QPoint &offset)
{
...
void XcbBackingStoreImage::flushPixmap(const QRegion &region, bool fullRegion)
{
    if (!fullRegion) {
...
void XcbBackingStoreImage::setClip(const QRegion &region)
{
    if (region.isEmpty()) {
...
void XcbBackingStoreImage::put(xcb_drawable_t dst, const QRegion &region, const QPoint &offset)
{
...
void XcbBackingStoreImage::preparePaint(const QRegion &region)
{
    if (hasShm()) {
...
bool MPlatformBackingStore::createSystemVShmSegment(xcb_connection_t *c, size_t segmentSize, void *shmInfo)
{
    auto info = reinterpret_cast<xcb_shm_segment_info_t *>(shmInfo);
    return XcbBackingStoreImage::createSystemVShmSegment(c, segmentSize, info);
}

MPlatformBackingStore::MPlatformBackingStore(QWindow *window)
    : QPlatformBackingStore(window)
{
    MPlatformScreen *screen = static_cast<MPlatformScreen *>(window->screen()->handle());
    setConnection(screen->connection());
}

MPlatformBackingStore::~MPlatformBackingStore()
{
    delete m_image;
}

QPaintDevice *MPlatformBackingStore::paintDevice()
{
    if (!m_image)
        return nullptr;
...
void MPlatformBackingStore::beginPaint(const QRegion &region)
{
    if (!m_image)
        return;
...
void MPlatformBackingStore::endPaint()
{
    ...
    MPlatformWindow *platformWindow = static_cast<MPlatformWindow *>(window()->handle());
...
QImage MPlatformBackingStore::toImage() const
{
...
QPlatformGraphicsBuffer *MPlatformBackingStore::graphicsBuffer() const
{
    return m_image ? m_image->graphicsBuffer() : nullptr;
}

void MPlatformBackingStore::flush(QWindow *window, const QRegion &region, const QPoint &offset)
{
...
void MPlatformBackingStore::render(xcb_window_t window, const QRegion &region, const QPoint &offset)
{
    m_image->put(window, region, offset);
}

void MPlatformBackingStore::composeAndFlush(QWindow *window, const QRegion &region, const QPoint &offset,
...
void MPlatformBackingStore::resize(const QSize &size, const QRegion &)
{
    ...
    MPlatformWindow* win = static_cast<MPlatformWindow *>(pw);

    recreateImage(win, size);
}

void MPlatformBackingStore::recreateImage(MPlatformWindow *win, const QSize &size)
{
    if (m_image)
        m_image->resize(size);
    else
        m_image = new XcbBackingStoreImage(this, size);
...
bool MPlatformBackingStore::scroll(const QRegion &area, int dx, int dy)
{
    if (m_image)
