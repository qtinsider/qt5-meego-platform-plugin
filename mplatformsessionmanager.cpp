...
static void sm_performSaveYourself(MPlatformSessionManager*);
                                  ...
                                  int saveType, Bool shutdown , int interactStyle, Bool /*fast*/)
{
    ...
    sm_performSaveYourself((MPlatformSessionManager*) clientData);
    if (!sm_isshutdown) // we cannot expect a confirmation message in that case
        resetSmState();
}

static void sm_performSaveYourself(MPlatformSessionManager *sm)
{
    if (sm_isshutdown)
...
static void sm_shutdownCancelledCallback(SmcConn smcConn, SmPointer clientData)
{
    ...
    if (sm_waitingForInteraction)
        ((MPlatformSessionManager *) clientData)->exitEventLoop();
    resetSmState();
}
...
// MSessionManager starts here

MPlatformSessionManager::MPlatformSessionManager(const QString &id, const QString &key)
...
MPlatformSessionManager::~MPlatformSessionManager()
{
    if (smcConnection)
...
void* MPlatformSessionManager::handle() const
{
    return (void*) smcConnection;
}

bool MPlatformSessionManager::allowsInteraction()
{
    if (sm_interactionActive)
        return true;
...
bool MPlatformSessionManager::allowsErrorInteraction()
{
    if (sm_interactionActive)
        return true;
...
void MPlatformSessionManager::release()
{
    if (sm_interactionActive) {
...
void MPlatformSessionManager::cancel()
{
    sm_cancel = true;
}

void MPlatformSessionManager::setManagerProperty(const QString &name, const QString &value)
{
    sm_setProperty(name, value);
}

void MPlatformSessionManager::setManagerProperty(const QString &name, const QStringList &value)
{
    sm_setProperty(name, value);
}

bool MPlatformSessionManager::isPhase2() const
{
    return sm_in_phase2;
}

void MPlatformSessionManager::requestPhase2()
{
    sm_phase2 = true;
}

void MPlatformSessionManager::exitEventLoop()
{
    m_eventLoop->exit();
}

#include "mplatformsessionmanager.moc"
